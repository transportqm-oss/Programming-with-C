...
void display10Year(Employee list[], int n);
void displayHeader();
		...
		do {
			...
			for(j = 0; j < i; j++) {
				if(list[j].emp_id == list[i].emp_id) {
					printf("  [!] ID %d already exists. Please enter another.\n", list[i].emp_id);
					duplicate = 1;
					break;
				}
			}
...
void increaseSalary(Employee list[], int n) {
	...
	for(i = 0; i < n; i++) {
		if(list[i].salary <= 2000) {
			increase = list[i].salary * 0.15f;	// up 15%
		} else if(list[i].salary <= 5000) {
			increase = list[i].salary * 0.10f;	// up 10%
		} else {
			increase = 0;
		}
