...
//define prototype
void inputAndSave(char filename[], int seiesNum);
...
int main() {
	...
	do {
		...
		printf("==========================================\n");
		printf("  1. Enter Series 1 (save to %s)\n", file1);
		printf("  2. Enter Series 2 (save to %s)\n", file2);
...
//declare function: input series and save file
void inputAndSave(char filename[], int seriesNum) {
	...
	//ghi so luong phan tu vao dong dau cua file
	fprintf(fp, "%d\n", count);
...
//declare function: merge 02 file, sort, luu vao file moi
void mergeAndSort(char file1[], char file2[], char fileOut[]) {
	FILE *fp1, *fp2, *fpOut;
	int arr[MAX * 2];
	...
	//doc file 02 noi tiep vao arr sau file 1
	fp2 = fopen(file2, "r");
	if(fp2 == NULL) {
		printf("  [!] Cannot open file %s.\n", file2);
		return;
