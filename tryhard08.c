...
//Function: equalsIgnoreCaseChar
int equalsIgnoreCaseChar(char a, char b) {
	return tolower((unsigned char) a) == tolower((unsigned char) b);
}

//Function: containIgnoreCase
int containIgnoreCase(const char *text, const char *keyword) {
...
void printBookNow(const Book *book) {
...
void searchBooks(void) {
	...
	printf("\n=== SEARCH BOOKS ===\n");
	readLine("Enter title, author or category keyword: ", keyword, sizeof(keyword));
	...
	for(i = 0; i < bookCount; i++) {
		if(containIgnoreCase(books[i].title, keyword) ||
		        containIgnoreCase(books[i].author, keyword) ||
		        containIgnoreCase(books[i].category, keyword)) {
			printBookNow(&books[i]);
			found = 1;
		}
	}
...
void sortBooksByTitle(void) {
	...
	printf("Books sorted title successfully.\n");
}
...
void borrowBook(void) {
	...
	if(books[index].available <= 0) {
		printf("No available copy this book.\n");
		return;
	}
	...
	getCurrentDate(loan.borrowerDate);
...
void returnBook(void) {
	...
	for(i = 0; i < loanCount; i++) {
		if(loans[i].id == loanId) {
			...
			if(bookIndex != -1 && books[bookIndex].available < books[bookIndex].quantity) {
...
void showStatistics(void) {
	...
	for(i = 0; i < bookCount; i++) {
		...
		if(mostBorrowedIndex == -1 || books[i].borrowCount > books[mostBorrowedIndex].borrowCount) {
			mostBorrowedIndex = i;
		}
	}
	...
	printf("Retuned loans        : %d\n", returnedLoans);
	printf("All loan transaction : %d\n", loanCount);
